...
app.post("/api/fix/remove-avatar-index", async (c) => {
  try {
    const databaseUrl = c.env?.DATABASE_URL;
    if (!databaseUrl) {
      return c.json({
    ...
    await prisma2.$executeRaw`
      DROP INDEX IF EXISTS "idx_users_avatar_data";
    `;
    return c.json({
      code: 200,
      message: "Avatar index removed successfully",
      data: {
        success: true,
        action: "Removed problematic index on avatarData field"
      }
    });
  } catch (error) {
