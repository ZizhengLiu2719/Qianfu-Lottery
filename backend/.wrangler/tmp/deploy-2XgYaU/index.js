...
app.post("/api/migrate/fix-avatar-fields", async (c) => {
  try {
    const databaseUrl = c.env?.DATABASE_URL;
    if (!databaseUrl) {
      return c.json({
    ...
    const prisma2 = getPrismaClient2(databaseUrl);
    const tableInfo = await prisma2.$queryRaw`
    ...
    try {
      await prisma2.$executeRaw`
        ALTER TABLE "qiancaidou_transactions" DROP COLUMN IF EXISTS "avatarData";
      `;
      await prisma2.$executeRaw`
        ALTER TABLE "qiancaidou_transactions" DROP COLUMN IF EXISTS "avatarMimeType";
      `;
      await prisma2.$executeRaw`
        ALTER TABLE "qiancaidou_transactions" DROP COLUMN IF EXISTS "avatarSize";
      `;
    } catch (dropError) {
      console.log("Drop columns error (may not exist):", dropError);
    }
    return c.json({
      code: 200,
      message: "Avatar fields migration fixed successfully",
      data: {
        success: true,
        fieldsAdded: ["avatarData", "avatarMimeType", "avatarSize"],
        targetTable: "users"
      }
    });
  } catch (error) {
